...
const TAIWAN_DATA: Record<string, Record<string, string>> = {
...
export default function TaiwanAddressSelector({
  zipcode, city, district, detail,
  onChangeZipcode, onChangeCity, onChangeDistrict, onChangeDetail,
}: Props) {
  const districts = useMemo(() => {
    if (!city) return {};
    return TAIWAN_DATA[city] || {};
  }, [city]);
  ...
  // 自動帶入郵遞區號
  useEffect(() => {
    if (city && district && districts[district]) {
      onChangeZipcode(districts[district]);
    }
  }, [city, district, districts, onChangeZipcode]);
